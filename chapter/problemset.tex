...
\item 质点沿 \(x\) 轴运动,其加速度和位置的关系为$a=2+6x^2\ \text{(SI单位)}.$质点在 \(x=0\) 处速度为 \(10\,\text{m}\cdot\text{s}^{-1}\),试求质点在任意坐标 \(x\) 处的速度值.
...
由于位移 $\Delta \mathbf{r}$ 是从起点指向终点的矢量，回到起点意味着总位移的大小为 $0$.因此，其平均速度大小由公式(1)得出:
\begin{equation}
...
(3) $\mathbf{\frac{ds}{dt} = v}$:式中 $s$ 是路程，$v$ 是速率.物理学中速率的确切定义就是路程对时间的一阶导数，即公式(1):
\begin{equation}
	v = \frac{ds}{dt}
\end{equation}
...
首先，对角位移方程求时间 $t$ 的一阶导数，得到质点做圆周运动的角速度 $\omega$ 的表达式，如公式(1):
\begin{equation}
	\omega = \frac{d\theta}{dt} = \frac{d}{dt}(3 + 2t^2) = 4t
\end{equation}

题目要求的是 $t$ 时刻质点的\textbf{法向加速度(向心加速度)}大小 $a_n$.根据圆周运动的法向加速度推导公式(2):
\begin{equation}
	a_n = \omega^2 R
\end{equation}

将公式(1)中求得的 $\omega$ 代入公式(2)中，推出最终法向加速度的表达式(3):
...
\end{equation}
由公式(2)可知总加速度的大小 $a = |\mathbf{a}| = 2$.
由加速度的分解公式 $a^2 = a_t^2 + a_n^2$，结合公式(4)可推导出法向加速度 $a_n$ 的表达式(5):
\begin{equation}
...
根据加速度的定义，并利用链式求导法则(将对时间的导数转化为对位移的导数)，得到公式(1):
\begin{equation}
...
先求角速度 $\omega$ 和角加速度 $\alpha$，分别得到公式(1)和公式(2):
\begin{equation}
...
当 $t = 2\text{ s}$ 时，由公式(1)和公式(2)得 $\omega = 9 \times 2^2 = 36 \text{ rad/s}$，$\alpha = 18 \times 2 = 36 \text{ rad/s}^2$.
切向加速度由公式(3)计算:
\begin{equation}
...
当总加速度与半径成 $45^\circ$ 角时，说明切向加速度和法向加速度的大小相等，建立等量关系式(5):
\begin{equation}
...
此时质点从 $t=0$ 到该时刻的偏转角(即角位移的增量)可由公式(6)计算:
\begin{equation}
...
对时间 $t$ 求导，分别得到速度的 $x$ 和 $y$ 方向分量，如公式(1)和公式(2)所示:
\begin{equation}
	v_x = \frac{dx}{dt} = 5
\end{equation}
\begin{equation}
...
\end{equation}
在 $t_1 = 2\text{ s}$ 时，由公式(3)得速率平方 $v_1^2 = 25 + 2^2 = 29$，初动能 $E_{k1} = \frac{1}{2} \times 0.5 \times 29 = 7.25\text{ J}$.
...
根据变力做功的定义式 $W = \int_{x_1}^{x_2} F_x dx$，对力在位移区间上进行定积分得到公式(1):
\begin{equation}
...
由 $F = ma = m\frac{dv}{dt}$，建立运动微分方程(1):
\begin{equation}
	m\frac{dv}{dt} = -kv
\end{equation}
...
利用微积分中的链式法则变换加速度式 $a = \frac{dv}{dt} = \frac{dv}{dx}\frac{dx}{dt} = v\frac{dv}{dx}$，代入动力学方程并分离变量，得出微分公式(4):
\begin{equation}
	m v \frac{dv}{dx} = -kv \implies \frac{dv}{dx} = -\frac{k}{m}
\end{equation}
将速度的上下限($v_0 \to 0$)和位移的上下限($0 \to x_{\text{max}}$)代入公式(4)进行积分:
...
由公式(3)可知，在 $t = 1\text{ s}$ 时，速度 $\mathbf{v} = 5\mathbf{i}$ 的方向仅沿 $x$ 轴.
结合公式(4)，总力的 $x$ 轴分量 $4\mathbf{i}$ 是\textbf{平行于速度的切向力}，而力的 $y$ 轴分量 $-24\mathbf{j}$ 则是\textbf{垂直于速度的法向力}.
...
(2) 求子弹所受的冲量:
根据冲量的定义式 $I = \int F dt$，在时间区间 $[0, \frac{a}{b}]$ 内对合力进行定积分，得到公式(2):
\begin{equation}
...
(3) 求子弹的质量:
根据动量定理，合外力的冲量等于子弹动量的变化量，由此建立公式(4):
\begin{equation}
...
(2) 求合力做功的平均功率:
平均功率定义为功除以时间，将上一步计算的功代入公式(2):
\begin{equation}
	\bar{P} = \frac{W}{t} = \frac{-45\text{ J}}{0.6\text{ s}} = -75\text{ W}
\end{equation}

(3) 求质点动能的变化:
根据动能定理，合外力对质点做的总功等于质点动能的变化量，由此推出关系式(3):
\begin{equation}
...
因为是匀速提升，人施加的拉力大小等于总重力，由此得出受力函数公式(1):
\begin{equation}
	F(y) = M(y)g = (11 - 0.2y)g
\end{equation}
...
对 $m_1$ 应用牛顿第二定律(向下为正)，结合公式(3)得到公式(5):
\begin{equation}
...
\end{equation}
将公式(8)展开并提取出 $\alpha$，推导出角加速度的表达式(9):
\begin{equation}
...
(2). \textbf{求两侧细绳的张力:}
将由公式(10)求得的 $\alpha$ 分别代回公式(5)和公式(6)，得出张力公式(11)和公式(12):
\begin{equation}
...
$m_1$ 下落时的恒定线加速度由公式(3)得 $a_1 = \alpha R = 6.125 \times 0.20 = 1.225 \text{ m/s}^2$.
根据匀变速直线运动规律，初速度为 $0$，位移 $h = 2 \text{ m}$，落地速率 $v_1$ 满足公式(13):
\begin{equation}
...
将公式(4)和公式(5)代入公式(3)中，消去 $J$ 和 $\alpha$，得到关于线加速度的转动方程(6):
\begin{equation}
	(T_2 - T_1)r = \left(\frac{1}{2}m'r^2\right) \left(\frac{a}{r}\right) \implies T_2 - T_1 = \frac{1}{2}m'a
\end{equation}

将公式(1)中的 $T_1$ 和公式(2)变形得到的 $T_2 = m_2 g - m_2 a$ 一并代入公式(6)中:
\begin{equation}
...
根据刚体转动惯量的计算，均匀细杆绕过其一端且垂直于杆的轴的转动惯量 $J$ 为公式(1):
\begin{equation}
	J = \frac{1}{3}ml^2
\end{equation}

(1) \textbf{求初始时刻的角加速度 $\alpha_0$:}
初始时刻，细杆处于静止的水平位置.此时重力 $mg$ 集中作用在杆的质心(距 $O$ 点距离为 $\frac{l}{2}$)上.此时重力的力臂最大，产生的重力力矩 $\tau_0$ 见公式(2):
\begin{equation}
	\tau_0 = mg \frac{l}{2}
\end{equation}
...
根据机械能守恒定律，系统的重力势能与转动动能之和为零，建立公式(8):
\begin{equation}
...
\end{equation}
对公式(10)中的 $\omega^2$ 开平方，解得此时的角速度大小(11):
\begin{equation}
...
\textbf{解答过程:}
(1). \textbf{求放手时棒的角加速度:}
设均匀直棒的质量为 $m$，长度为 $L = 1\text{ m}$.棒绕一端固定的水平光滑轴转动，其转动惯量由公式(1)给出:
\begin{equation}
	J = \frac{1}{3}mL^2
\end{equation}
放手瞬间，棒受到的重力力矩由重力在垂直于棒的方向上的分量产生.重力作用于棒的质心(距转轴 $\frac{L}{2}$ 处)，且题图所示棒与水平面的夹角为 $60^\circ$，因此重力力臂为 $\frac{L}{2} \cos 60^\circ$.由此建立重力力矩 $\tau$ 的公式(2):
\begin{equation}
	\tau = mg \frac{L}{2} \cos 60^\circ
\end{equation}
根据刚体定轴转动定律 $\tau = J\alpha$，结合公式(1)和公式(2)，可得角加速度 $\alpha$ 为公式(3):
\begin{equation}
...
设杆的转动惯量为 $J$，质量为 $m$，长度为 $L$.小球质量 $m_0 = m/3$，碰撞点 $OP = \frac{3}{4}L$.转动惯量由公式(1)给出:
\begin{equation}
	J = \frac{1}{3}mL^2
\end{equation}
...
\end{equation}
碰撞后，小球以 $v = v_0/3$ 的速度反弹，杆以角速度 $\omega_0$ 开始转动.碰撞后的总角动量为公式(3):
\begin{equation}
...
\end{equation}
对公式(5)移项化简，求得开始转动时的角速度 $\omega_0$(6):
\begin{equation}
...
此摩擦力对转轴 $O$ 产生的微摩擦力矩为 $dM_f = x df$，建立微元公式(7):
\begin{equation}
...
杆在定摩擦力矩作用下做匀减速转动.根据刚体定轴转动定律，由公式(1)和公式(8)求得角加速度 $\alpha$ 的大小(9):
\begin{equation}
...
\end{equation}
转过的角度 $\theta$ 可由 $\omega_t^2 - \omega_0^2 = -2|\alpha|\theta$ 得到公式(11):
\begin{equation}
...
碰撞前，细棒静止，只有小球具有相对于 $O$ 轴的角动量，如公式(1):
\begin{equation}
	L_i = m_2 v_1 l
\end{equation}
碰撞后，细棒以角速度 $\omega$ 转动，小球反弹，速度方向相反，取值为 $-v_2$.碰撞后系统的总角动量如公式(2):
\begin{equation}
...
\end{equation}
对公式(4)移项并解得碰撞后瞬间细棒的角速度 $\omega$(5):
\begin{equation}
...
\end{equation}
将公式(3)的 $J$ 和公式(5)的 $\omega$ 代入公式(8)中:
\begin{equation}
...
\end{equation}
由公式(12)应用反三角函数最终得到最大摆角 $\theta_m$(13):
\begin{equation}
...
从图中可以看出，$\theta$ 是细杆与水平线的夹角.重力作用在杆的质心(距 $O$ 点 $l/2$ 处)，力臂为 $\frac{l}{2}\cos\theta$.重力产生的力矩 $\tau$ 如公式(1)所示:
\begin{equation}
...
细杆从水平位置自由转到竖直位置，质心下降的高度为 $l/2$.根据机械能守恒，减小的重力势能转化为细杆的转动动能 $E_k$，如公式(3):
\begin{equation}
	E_k = m_1 g \frac{l}{2}
\end{equation}
由 $E_k = \frac{1}{2}J\omega^2$ 可求得此时细杆的角速度 $\omega$(4):
\begin{equation}
...
碰撞过程时间极短，系统关于 $O$ 轴的角动量守恒.设碰后杆的角速度为 $\omega'$.由角动量守恒建立公式(6):
\begin{equation}
...
\end{equation}
由公式(6)可得 $J(\omega - \omega') = m_2 v' l$.
由公式(7)可得 $J(\omega - \omega')(\omega + \omega') = m_2 v'^2$.
...
\end{equation}
由公式(13)解得弹簧的最大压缩量 $\Delta x$ 为公式(14):
\begin{equation}
